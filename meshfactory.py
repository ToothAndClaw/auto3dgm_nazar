...
class MeshFactory(object):
    ...
    def mesh_from_file(file_path, center_scale=False):
        ...
        if isfile(file_path) and splitext(file_path)[1] in allowed_filetypes:
            ...
            filebasename=os.path.splitext(file_path)[0]
            ...
            raise OSError(msg)
        return Mesh(vtk_mesh=polydata, center_scale=center_scale,filebasename)

    @staticmethod
    def mesh_from_data(vertices, faces=empty([0,0]), center_scale=False, deep=True,hashname=None):
        ...
        if hashname==None:
            hashname=np.random.bytes(10)#random.getrandbits(128)   

        return Mesh(vtk_mesh=polydata, center_scale=center_scale,hashname)
